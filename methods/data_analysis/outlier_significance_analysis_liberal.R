...
analyze_outlier_significance <- function(k = 100, B = 1000, output_dir = NULL) {
  ...
  for (current_norm in unique(all_genes_table$norm_method)) {
    ...
    # BH-Korrektur für noise_p_value (pro cancer_id, norm, stage, comp) - liberal
    norm_data <- norm_data %>%
...
if (sys.nframe() == 0) {
  set.seed(42)
  base_dir <- file.path(dirname(FAMILY_OUTPUT_DIR), "outlier_significance_liberal_grouping")
  
  # for now
  k_values <- c(25, 50, 75, 100, 200)
  B_values <- c(1000, 500, 750)
